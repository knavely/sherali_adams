...
def fill_look(k,n, look = {}):
    ...
    for i in range(k):
        lk = {}
        sm = sm + comb(n,i+1)
        for (u,v) in itertools.combinations(range(sm),2):
...
def sum_comb(k,n):
    return sum(map(lambda i: comb(n,i), range(1,k+1)))
...
def get_SA_instance(rnd,n,oldA,oldb, look = {}):
    ...
    oldN = cols
    newN = sum(map(lambda j: comb(n,j),range(1,rnd+2)))
    ...
    for k in range(n):
        ...
        for r in range(rows):
            for c in range(cols):
                j = extend_index((c,k),n,rnd) if not look else extend_index_mem(look,(c,k),n,rnd)
                if(oldA[r,c] != 0):
                    Ak[r,j] += oldA[r,c]
                    nAk[r,c] += oldA[r,c]
                    nAk[r,j] += -oldA[r,c]
