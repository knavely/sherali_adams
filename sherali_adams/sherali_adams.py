...
# for testing.....
def find_triangles(nvariables, trips):
    triangles = [] #np.zeros(10)   
    for (a,b,c) in trips:
        #if (((a,b) in oriented and (b,c) in oriented and (c,a) in oriented) or ((c,b) in oriented and (b,a) in oriented and (a,c) in oriented)):
            r = np.zeros(nvariables)
            r[a] = 1
            r[b] = 1
            r[c] = 1
            triangles.append(r)
...
def to_digraph_trips(ag,n):
    tr = [(a,b,c) for (a,b,c) in itertools.combinations(range(n),3)
          if ((ag[a,b] == 1 and ag[b,c] == 1 and ag[c,a] == 1) or
              (ag[a,c] == 1 and ag[c,b] == 1 and ag[b,a] == 1) or
              (ag[b,a] == 1 and ag[a,c] == 1 and ag[c,a] == 1) or
              (ag[b,c] == 1 and ag[c,a] == 1 and ag[a,b] == 1) or
              (ag[c,a] == 1 and ag[a,b] == 1 and ag[b,c] == 1) or
              (ag[c,b] == 1 and ag[b,a] == 1 and ag[a,c] == 1))
          ]
    return tr

def get_digraph_instance(n):
    di = sage.all.digraphs.RandomTournament(n)
    trips = to_digraph_trips(di.adjacency_matrix(),n)
    tri = find_triangles(n, trips)    
    A = np.matrix(-np.asarray(tri))
    b = -np.ones(len(tri))

    xx_min = 0 * np.ones(n)
    xx_max = 1* np.ones(n)

    h = np.hstack([+xx_max, -xx_min])
    c = np.ones(n)
    G = np.vstack([
     np.hstack([+np.eye(n)]),
     np.hstack([-np.eye(n)])])

    return (c,np.matrix(np.vstack([A,G])),np.hstack([b,h]))
